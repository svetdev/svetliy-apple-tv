...
@synthesize currentInlineAd = _currentInlineAd;
@synthesize currentInlineAdPlayerItem = _currentInlineAdPlayerItem;
@synthesize adsQueue = _adsQueue;
...
- (void)playInlineAd:(DVInlineVideoAd *)videoAd
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"removeAdTimer" object:nil];
    
    AVPlayerItem *playerItem = [[AVPlayerItem alloc] initWithURL:videoAd.mediaFileURL];
    /* svet hack*/
    
    if (!videoAd.mediaFileURL){
        [self finishCurrentInlineAd:nil];
        return;
    }
    [[NSNotificationCenter defaultCenter] addObserver:self
...
//        VLogF(current);
    ...
    // acceptInvitation
    // close
    if (self.currentInlineAd != nil)
    {
        if (self.currentInlineAd.isSkippable)
        {
            [[NSNotificationCenter defaultCenter] postNotificationName:@"setupAdTimer" object:nil];
        }
    }
