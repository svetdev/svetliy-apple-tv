...
class BaseCollectionVC: UICollectionViewController {
  ...
  override func viewDidAppear(animated: Bool) {
    ...
    if(self.isInfinityScrolling) {
      self.timer = NSTimer.scheduledTimerWithTimeInterval(BaseCollectionVC.autoscrollInterval, target: self, selector: #selector(BaseCollectionVC.scrollToNextItem), userInfo: nil, repeats: true)
    }
  }
